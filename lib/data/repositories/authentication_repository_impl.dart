...
class AuthenticationRepositoryImpl extends AuthenticationRepository {
  ...
  Future<Either<AppError, bool>> loginUser(Map<String, dynamic> body) async {
    ...
    final token1 =
        requestTokenEitherResponse.getOrElse(() => null)?.requestToken ?? '';//seession id

    try {
      body.putIfAbsent('request_token', () => token1);
      final validateWithLoginToken =
          await _authenticationRemoteDataSource.validateWithLogin(body);//second call with empty request token return empty request token
      final sessionId = await _authenticationRemoteDataSource
          .createSession(validateWithLoginToken.toJson());//last api return false
      if (sessionId != null) {
